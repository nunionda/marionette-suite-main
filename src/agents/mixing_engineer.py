...
class MixingEngineerAgent:
    ...
    def _find_best_video(self) -> str | None:
    ...
    def _find_bgm_track(self) -> str | None:
        """BGM 트랙 찾기"""
    ...
    def mix_final(self, title: str = "bit_savior") -> str | None:
        """
        최종 믹싱 — 비디오 + 대사 오디오 합본
    ...
    def _merge_video_audio(self, video: str, audio: str, output: str) -> str | None:
        """비디오 + 단일 오디오 트랙 합본"""
        cmd = [
            "ffmpeg", "-y",
            "-i", video,
    ...
    def _merge_video_dialogue_bgm(self, video: str, dialogue: str, bgm: str, output: str) -> str | None:
        """비디오 + 대사 + BGM 3트랙 믹싱"""
        cmd = [
            "ffmpeg", "-y",
            "-i", video,
    ...
    def _copy_video(self, video: str, output: str) -> str | None:
        """오디오 없이 비디오만 복사"""
        cmd = ["ffmpeg", "-y", "-i", video, "-c", "copy", output]
        return self._run_ffmpeg(cmd, output)

    def _run_ffmpeg(self, cmd: list, output: str) -> str | None:
        try:
