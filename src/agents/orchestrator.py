...
class OrchestratorAgent:
    ...
    def _run_production_pipeline(self, json_path: str, active_steps: list[str] = None):
        """
        프로덕션 파이프라인 공통 실행부 (Step 2~7)
        """
        all_steps = active_steps or ["storyboard", "video", "asset", "vfx", "edit", "sound"]

        # Step 2: ConceptArtist — 스토리보드 이미지 생성
        if "storyboard" in all_steps:
            print("🚀 [Step 2/7] AI 컨셉 아티스트 (ConceptArtist)")
        ...
        # Step 3: Generalist — 비디오 푸티지
        if "video" in all_steps:
            print("🚀 [Step 3/7] AI 제너럴리스트 (Generalist)")
            GeneralistAgent().generate_videos(json_path)
            print()

        # Step 4: AssetDesigner — 3D 에셋
        if "asset" in all_steps:
            print("🚀 [Step 4/7] AI 에셋 디자이너 (AssetDesigner)")
            AssetDesignerAgent().generate_3d_assets(json_path)
            print()

        # Step 5: VFXCompositor — VFX 합성
        if "vfx" in all_steps:
            print("🚀 [Step 5/7] AI VFX 컴포지터 (VFXCompositor)")
            VFXCompositorAgent().apply_vfx_and_composite(json_path)
            print()

        # Step 6: MasterEditor — 편집
        if "edit" in all_steps:
            print("🚀 [Step 6/7] AI 마스터 에디터 (MasterEditor)")
            MasterEditorAgent().edit_and_color_grade(json_path)
            print()

        # Step 7: SoundDesigner — 사운드
        if "sound" in all_steps:
            print("🚀 [Step 7/7] AI 사운드 디자이너 (SoundDesigner)")
            SoundDesignerAgent().generate_audio(json_path)
            print()

        print("🎬 마리오네트 파이프라인 완료! 🎬\n")
