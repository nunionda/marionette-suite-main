...
export class TropeAnalyzer {
   ...
   */
  async analyze(scriptId: string, elements: ScriptElement[]): Promise<TropeResult> {
    // Build a synopsis from the first ~30 scenes of action + dialogue
    const synopsis = this.buildSynopsis(elements);

    const systemPrompt = `You are a trope analyzer for Hollywood screenplays.
...
${TROPE_DICTIONARY.join(", ")}

Respond with a JSON array of strings. Example: ["Revenge", "Anti-Hero", "Twist Ending"]
      ...
      if (Array.isArray(parsed)) {
        ...
        const valid = parsed.filter(
          (t: unknown): t is string =>
            typeof t === "string" &&
            TROPE_DICTIONARY.some((d) => d.toLowerCase() === (t as string).toLowerCase()),
        );
