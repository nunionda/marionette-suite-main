...
const PORT = Number(process.env.CONTENTS_STUDIO_API_PORT ?? process.env.PORT ?? 3005)
...
app.use("*", cors({
  origin: (origin) => {
    // Allow configured origins + any localhost port (for dev auto-port)
    const fallback = process.env.WEB_ORIGIN || CORS_ORIGINS[0] || "http://localhost:3000"
