...
authRoutes.get("/me", async (c) => {
  ...
  if (!token && process.env.NODE_ENV !== "production") {
    const { findAdminForDev } = await import("../services/auth.service.ts")
    const admin = await findAdminForDev()
    if (admin) {
      const devToken = await createToken(admin)
      setCookie(c, "token", devToken, COOKIE_OPTIONS)
