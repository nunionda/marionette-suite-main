"use client";

import { useEffect, useMemo, useState } from "react";
import type { DCPPackage } from "../../../lib/dcp/mock-entries";
import { STATUS_COLOR, STATUS_LABEL } from "../../../lib/dcp/mock-entries";

interface Props {
  packages: DCPPackage[];
}

export function DCPClient({ packages }: Props) {
  const [highlightedProjectId, setHighlightedProjectId] = useState<string | null>(null);

  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const pid = params.get("paperclipId");
    if (pid) setHighlightedProjectId(pid);
  }, []);

  const byProject = useMemo(() => {
    const map = new Map<string, DCPPackage[]>();
    for (const p of packages) {
      const arr = map.get(p.projectId) ?? [];
      arr.push(p);
      map.set(p.projectId, arr);
    }
    return map;
  }, [packages]);

  const projectIds = useMemo(() => {
    const ids = Array.from(byProject.keys());
    if (highlightedProjectId && ids.includes(highlightedProjectId)) {
      return [highlightedProjectId, ...ids.filter((id) => id !== highlightedProjectId)];
    }
    return ids;
  }, [byProject, highlightedProjectId]);

  return (
    <div className="flex flex-col gap-6">
      <header className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-wider text-white">DCP MASTERING</h1>
          <p className="mt-1 text-xs text-neutral-500">디지털 시네마 패키지 · 마스터링 현황</p>
        </div>
        <a href="/projects" className="text-xs text-neutral-500 underline hover:text-neutral-300">
          ← Projects
        </a>
      </header>

      {projectIds.length === 0 && (
        <p className="text-sm text-neutral-500">등록된 DCP 패키지가 없습니다.</p>
      )}

      {projectIds.map((pid) => {
        const projectPackages = byProject.get(pid) ?? [];
        const isHighlighted = pid === highlightedProjectId;
        const delivered = projectPackages.filter(
          (p) => p.status === "delivered" || p.status === "screened",
        ).length;

        return (
          <section
            key={pid}
            className="rounded-lg border p-5"
            style={{
              borderColor: isHighlighted ? "#00FF41" : "#1E1E1E",
              backgroundColor: "#0F0F0F",
              boxShadow: isHighlighted ? "0 0 0 1px #00FF4133" : undefined,
            }}
          >
            <header className="mb-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <h2 className="font-mono text-sm font-bold tracking-wider text-white">{pid}</h2>
                <span className="text-[10px] uppercase tracking-wider text-neutral-500">
                  {projectPackages.length}개 패키지 · 납품 {delivered}/{projectPackages.length}
                </span>
              </div>
              {isHighlighted && (
                <span
                  className="rounded px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider"
                  style={{ color: "#00FF41", border: "1px solid #00FF4144", backgroundColor: "#00FF4111" }}
                >
                  deep-linked
                </span>
              )}
            </header>

            <table className="w-full text-left text-xs">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-neutral-600">
                  <th className="pb-2 font-medium">버전</th>
                  <th className="pb-2 font-medium">포맷</th>
                  <th className="pb-2 font-medium">해상도</th>
                  <th className="pb-2 text-right font-medium">화면비</th>
                  <th className="pb-2 text-right font-medium">CH</th>
                  <th className="pb-2 text-right font-medium">크기</th>
                  <th className="pb-2 font-medium">납품처</th>
                  <th className="pb-2 font-medium">상태</th>
                </tr>
              </thead>
              <tbody>
                {projectPackages.map((p) => (
                  <tr key={p.id} className="border-t" style={{ borderColor: "#1E1E1E" }}>
                    <td className="py-2 text-neutral-300">{p.version}</td>
                    <td className="py-2 font-mono text-[11px] text-neutral-400">{p.format}</td>
                    <td className="py-2 font-mono text-[11px] text-neutral-400">{p.resolution}</td>
                    <td className="py-2 text-right font-mono text-[11px] text-neutral-400">{p.aspectRatio}</td>
                    <td className="py-2 text-right font-mono text-[11px] text-neutral-400">{p.audioChannels}ch</td>
                    <td className="py-2 text-right font-mono text-[11px] text-neutral-500">
                      {p.fileSizeGB !== undefined ? `${p.fileSizeGB}GB` : "—"}
                    </td>
                    <td className="py-2 text-[11px] text-neutral-500">
                      {p.deliveredTo ?? p.masteringHouse ?? "—"}
                    </td>
                    <td className="py-2">
                      <span
                        className="rounded px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider"
                        style={{
                          color: STATUS_COLOR[p.status],
                          border: `1px solid ${STATUS_COLOR[p.status]}44`,
                          backgroundColor: `${STATUS_COLOR[p.status]}11`,
                        }}
                      >
                        {STATUS_LABEL[p.status]}
                      </span>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>

            {projectPackages.some((p) => p.note) && (
              <div className="mt-3 space-y-1">
                {projectPackages.filter((p) => p.note).map((p) => (
                  <p key={p.id} className="text-[11px] text-neutral-600">
                    <span className="text-neutral-500">{p.version}:</span> {p.note}
                  </p>
                ))}
              </div>
            )}
          </section>
        );
      })}
    </div>
  );
}
