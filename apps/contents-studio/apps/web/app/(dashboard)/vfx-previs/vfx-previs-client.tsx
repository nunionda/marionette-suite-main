...
import type { VfxPrevisShot } from "../../../lib/vfx-previs/mock-entries";
import {
  COMPLEXITY_COLOR,
  COMPLEXITY_LABEL,
  STATUS_COLOR,
  STATUS_LABEL,
  VFX_TYPE_LABEL,
} from "../../../lib/vfx-previs/mock-entries";

interface Props {
  shots: VfxPrevisShot[];
}

export function VfxPrevisClient({ shots }: Props) {
  ...
  const byProject = useMemo(() => {
    const map = new Map<string, VfxPrevisShot[]>();
  ...
  return (
    <div className="flex flex-col gap-6">
      <header className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-wider text-white">VFX PREVIS</h1>
          <p className="mt-1 text-xs text-neutral-500">VFX 프리비즈 · 시각효과 샷 기획</p>
        </div>
      ...
      {projectIds.map((pid) => {
        ...
        const totalFrames = projectShots.reduce((n, s) => n + s.frameCount, 0);
            ...
            <header className="mb-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] uppercase tracking-wider text-neutral-500">
                  {projectShots.length}개 샷 · 히어로 {heroShots} · 총{" "}
                  <span className="font-mono">{totalFrames.toLocaleString()}</span>프레임
                </span>
              </div>
              {isHighlighted && (
                <span
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-neutral-600">
                  <th className="pb-2 font-medium">샷 ID</th>
                  <th className="pb-2 font-medium">VFX 유형</th>
                  <th className="pb-2 font-medium">복잡도</th>
                  <th className="pb-2 font-medium">소프트웨어</th>
                  <th className="pb-2 text-right font-medium">프레임</th>
              ...
              <tbody>
                {projectShots.map((s) => (
                  <tr key={s.id} className="border-t" style={{ borderColor: "#1E1E1E" }}>
                    <td className="py-2 font-mono text-[11px] text-neutral-300">{s.shotId}</td>
                    <td className="py-2 text-[11px] text-neutral-400">{VFX_TYPE_LABEL[s.vfxType]}</td>
                    <td className="py-2">
                      <span
                        className="rounded px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider"
                        style={{
                    ...
                    <td className="py-2 text-[11px] text-neutral-500">{s.software.join(", ")}</td>
                    ...
                    <td className="py-2 text-[11px] text-neutral-500">{s.dueDate ?? "—"}</td>
                    <td className="py-2">
                      <span
                        className="rounded px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider"
                        style={{
              ...
              <div className="mt-3 space-y-1">
                {projectShots.filter((s) => s.notes).map((s) => (
                  <p key={s.id} className="text-[11px] text-neutral-600">
                    <span className="text-neutral-500">{s.shotId}:</span> {s.notes}
                  </p>
                ))}
              </div>
            )}
