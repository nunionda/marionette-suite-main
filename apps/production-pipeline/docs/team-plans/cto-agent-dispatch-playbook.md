...
팀 에이전트(Backend Dev, Frontend Dev, DevOps, QA)를 `--enable-auto-mode`로 실행하면 최대 자율성을 얻지만 호스트 시스템이 위험해진다. Vagrant VM을 샌드박스로 사용하면:
...
└── 에이전트 디스패치 → Vagrant VM (Ubuntu 24.04)
        ...
        │
        ├── claude --enable-auto-mode -p "Backend Dev: ..."
        ├── claude --enable-auto-mode -p "Frontend Dev: ..."
        ├── claude --enable-auto-mode -p "DevOps: ..."
        └── claude --enable-auto-mode -p "QA: ..."
```
...
```bash
cd /agent-workspace
claude --enable-auto-mode -p "<팀원 역할 + 태스크>"
```
...
tmux send-keys -t agents "cd /agent-workspace && claude --enable-auto-mode -p 'You are Backend Dev...'" Enter

# Frontend Dev (window 1)
tmux new-window -t agents
tmux send-keys -t agents "cd /agent-workspace && claude --enable-auto-mode -p 'You are Frontend Dev...'" Enter

# DevOps (window 2)
tmux new-window -t agents
tmux send-keys -t agents "cd /agent-workspace && claude --enable-auto-mode -p 'You are DevOps...'" Enter
