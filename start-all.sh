...
# Ports Map
# 3000: Studio Hub | 3001: Homepage | 3005: Auteur API | 8000: Production Engine
...
# 3. Backends
start_service "Script Writer BE (3006)" "apps/script-writer/server" "$BUN_BIN run src/index.ts" "backend.log" "backend.pid"
start_service "Analysis System BE (4005)" "apps/analysis-system/server" "$BUN_BIN run src/index.ts" "backend.log" "backend.pid"

# 4. Frontends
start_service "Studio Hub (3000)" "apps/studio" "$PNPM_BIN dev" "studio.log" "studio.pid"
start_service "Homepage (3001)" "apps/homepage" "npx serve -l 3001 ." "homepage.log" "homepage.pid"
start_service "Script Writer FE (5174)" "apps/script-writer" "$PNPM_BIN dev" "frontend.log" "frontend.pid"
start_service "Analysis System FE (4000)" "apps/analysis-system" "$PNPM_BIN dev" "frontend.log" "frontend.pid"
